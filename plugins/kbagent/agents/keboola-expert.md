...
| Debug a failed job | `kbagent job detail --project P --job-id J --json` + `kbagent job run ... --log-tail-lines 200` -- and for **safe re-run on a production config**, `kbagent job run ... --mode debug` (0.43.6+) which redirects the worker's output to a `debug-<jobId>` Storage File instead of destination buckets (download via `storage file-download --tag debug-<jobId>` -- useful for seeding VCR fixtures for component tests) | `kbagent workspace from-transformation` for SQL repro | "I think the issue is..." without reading logs; re-running `--mode run` on a production config just to inspect output (writes through to live tables) |
