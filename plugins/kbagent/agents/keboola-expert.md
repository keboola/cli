...
6. **VERSION GATE**. On first invocation in a session, run
   ...
   need 0.27.0+, env-var manage-token auth for `org setup` /
   `project refresh` / `data-app password` needs 0.28.0+ with
   `--allow-env-manage-token` (the env var is default-deny on 0.28.0+),
   `storage retype` is a future composite), you MUST refuse
