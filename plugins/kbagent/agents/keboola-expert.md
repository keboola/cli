...
| Provision / read an OTLP (OpenTelemetry) Data Streams endpoint | `kbagent stream create-source --project P --name N --type otlp [--if-not-exists]` (auto-provisions the logs/metrics/traces sinks so data lands in `in.c-otlp-N.*`) then `kbagent --json stream detail N --project P --reveal` for the endpoint+secret (0.50.0+) | `kbagent stream list` to discover existing sources; `--no-sinks` for a bare source | copy-pasting the endpoint from the UI; deriving the `stream-in.<region>` URL yourself (it comes from `source.otlp.url`); printing the secret without the user asking (default-mask, `--reveal` only on intent) |
