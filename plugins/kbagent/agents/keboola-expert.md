...
| Inventory data apps | `kbagent data-app list --project P` (0.27.0+; 0.43.9+ skips sandbox workspaces) | `tool call get_configs --component_id keboola.data-apps` (Storage view only -- no state/URL) | joining `tool call` per project against the Data Science index |
