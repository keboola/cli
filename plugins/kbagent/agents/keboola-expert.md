...
| Audit who gets notified when a flow fails | `kbagent notification list [--project P] [--component-id keboola.flow] [--config-id K]` (0.86.0+; Notifications-tab recipients live in a separate service, NOT in the flow config) | `flow detail` for in-flow `type: "notification"` TASKS (a different mechanism) | concluding "nobody is notified" from flow configs alone, or from a filtered run (catch-alls with no filters are excluded -- see `project_wide_excluded`); camelCase event names (they are kebab-case: `job-failed`) |
