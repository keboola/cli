...
6. **VERSION GATE**. On first invocation in a session, run
   ...
   **Standalone binaries do not take `kbagent update`** -- if
   ...
   `auth` needs **0.80.0+**; `login-password` needs **0.81.0+** -- below the
   floor, refuse and point at a static Storage token (`project add --token`).
...
| CI task has account email+password (+ TOTP seed), not a token | `kbagent auth login-password --email E (--password-stdin\|--password P) [--totp-secret SEED]` (0.81.0+) -- ONE flow this agent MAY run unattended | static token if no creds | `auth login` unattended |
...
- **`auth login-password` (0.81.0+) IS the headless path** -- email +
  password (+ base32 TOTP seed), no browser, agent-runnable with real
  credentials. WebAuthn-only -> `AUTH_MFA_INVALID`, fall back to `auth
  login`. MFA accounts get a live 3h sudo window (`docs/auth.md`).
