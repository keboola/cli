...
6. **VERSION GATE**. On first invocation in a session, run
   ...
   needs 0.41.9+,
   `workspace list --branch` / `workspace list --qs-compatible` /
   `workspace detail --branch` (data-app discoverability fix; surfaces
   `login_type`, `read_only`, `qs_compatible` on workspace responses;
   `Info: production branch for read` banner parity with `storage
   buckets`; `config detail --component-id keboola.sandboxes` annotates
   the misleading `parameters.id` with the real `storage_workspace_id`)
   needs 0.42.0+,
...
| Find a Query-Service-compatible workspace for a data-app | `kbagent --json workspace list --project P --qs-compatible` (0.42.0+, closes #304) -- pre-filters to RO + whitelisted loginType (`snowflake-service-keypair` / `snowflake-person-sso`). If no rows: pick a `qs_compatible=true` row from plain `workspace list`, or fall back to `workspace create --read-only` and use its credentials | plain `workspace list` + manual eyeballing of the new `login_type` / `RO` / `QS` columns | spawning a probe SELECT 1 query against random workspace IDs to discover compatibility (pre-0.42.0 the only signal) -- the new fields removed that loop |
| Resolve a `keboola.sandboxes` config's real Storage workspace ID | `kbagent --json config detail --project P --component-id keboola.sandboxes --config-id K` (0.42.0+, closes #304) -- read `sandbox_annotation.storage_workspace_id`; pass that to `workspace detail --workspace-id`. NEVER use `configuration.parameters.id` -- it is the sandbox-service internal handle and 404s on the workspaces endpoint | `kbagent --json workspace list --project P` + filter where `config_id == <K>` (slower; the annotation IS this lookup, pre-computed) | passing `configuration.parameters.id` to `workspace detail --workspace-id` (returns 404; the field looks like a workspace ID but is not) |
