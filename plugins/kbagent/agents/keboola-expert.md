...
| Inventory data apps | `kbagent data-app list --project P` (0.27.0+; 0.43.9+ filters out `keboola.sandboxes` workspace deployments the Data Science `/apps` collection also returns) | `tool call get_configs --component_id keboola.data-apps` (Storage view only -- no state, no URL, no configVersion) | iterating `tool call` per project to reconstruct the join with the Data Science index |
...
| Read one secret / env-var key | `kbagent data-app secrets-get --project P --app-id N --key 'KEY'` (0.43.9+: `#` optional) -- ENCRYPTED key returns metadata only (`encrypted: true`, `value: null`); PLAIN key returns the literal value (`encrypted: false`) | -- | passing `--key '#KEY'` when the key is stored WITHOUT `#` (exact-match lookup -> NOT_FOUND); trying to decrypt an ENCRYPTED secret (impossible by design; not a CLI gap) |
| Remove a secret / env-var key from a data app | `kbagent data-app secrets-remove --project P --app-id N --key 'KEY' --yes` (0.43.9+: `#` optional, removes encrypted secrets AND plain env vars) -- idempotent; missing keys exit 0 with `removed: 0` | `tool call update_config` with the secrets sub-dict deleted -- ONLY for batch removes that need a custom change description | `kbagent config update --set 'parameters.dataApp.secrets={}'` -- replaces the whole sub-dict, dropping every secret instead of just the named ones |
...
- **`data-app secrets-* metadata-only`** (0.29.0+; plain-key behavior
  since 0.43.9): `secrets-get` on an ENCRYPTED key (`KBC::` ciphertext)
  NEVER echoes the decrypted plaintext under any branch -- the Encryption
  API is one-way and the CLI does not attempt to decrypt. For a PLAIN
  (unencrypted) env-var key the literal value IS returned
  (`encrypted: false`); that value is already visible via `config detail`,
  so this is not a new leak. `secrets-get`/`secrets-remove` accept keys
  with or without a leading `#` (the `parameters.dataApp.secrets` block
  holds both encrypted secrets and plain env vars; `secrets-set` still
  requires `#` because it encrypts). NOT_FOUND on an
