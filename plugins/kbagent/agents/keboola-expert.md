...
| Inspect dev branch / know where writes go | `kbagent branch current --project P` (vNEXT+; offline, id + name) BEFORE any write or long job; `kbagent branch list --project P`, `kbagent branch use --project P --branch ID`. A `branch use` pin persists for DAYS; every branch-aware command (config reads AND writes, `job run`, flow writes) targets it. vNEXT+ echoes it: stderr `Info: Using active dev branch ...` in human mode, a top-level `branch` key beside `data` in `--json` (`source` explicit/active/production; ABSENT = not branch-scoped, never = production) -- read `branch.id` before trusting a `--dry-run` | `kbagent doctor` (WARNs on a pinned project); a config `version` far below the UI's is the other tell (counters are per branch) | acting on `main` when a dev branch exists; assuming production because no message appeared (pre-vNEXT `config update` said nothing in ANY mode) |
