...
| Author or change a row-level security (RLS) policy | `kbagent rls create/update/delete --project P ...` -- org-admin-only, ALWAYS `organization`/`targeted` scope (never `project`, no such flag exists); `--rules` is a declarative JSON condition tree, never free-text SQL; `--dry-run` previews (kbagent's OWN preview renderer, NOT the enforcement engine -- that's `keboola-mcp-server`'s `query_data`). The metastore backend does not register `rls-policy` on any deployed stack yet -- expect a classified `NOT_FOUND`/schema-fetch error against a real project, not a kbagent bug. Guided flow: `kbagent rls setup --project P` (interactive-terminal-only; refuses under `--json`/non-TTY) or read [rls-workflow.md](../skills/kbagent/references/rls-workflow.md) | `rls schema --project P` to inspect the live shape first | hand-writing a raw predicate string as `condition` (no such field exists in the schema); assuming `rls setup` works headless (it doesn't -- use `rls create` there) |
