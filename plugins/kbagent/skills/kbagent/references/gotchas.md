...
## Programmatic auth (browser login) is human-only; sentinel tokens; v1 scope (since v0.78.0)
...
- **Registered aliases derive from the project NAME, never the numeric
  ...
  ...] [--alias ID=ALIAS ...] [--yes]` (0.78.0+) registers an EXISTING
  ...
  silently skipped, unlike the original 0.78.0 `--register-projects` batch
...
- **`auth logout --remove-projects` needs the `admin` permission class, the
  bare `auth logout` only `write`** (since v0.78.0). The flag deletes
...
- **An older (pre-0.78.0) kbagent build has no sentinel awareness at all.** A
...
message (since v0.78.0).** In `data-app list`, `tool list`, `tool call` and
...
| 3 | Authentication error (invalid or expired token) -- includes `SESSION_EXPIRED` / `SESSION_NOT_FOUND` / `AUTH_FLOW_DENIED`, whose remedy is `kbagent auth login` (since v0.78.0) |
| 4 | Network error (timeout, unreachable) -- includes `QUEUE_JOB_TIMEOUT` (local gave up AND the remote-kill attempt failed; the remote job may still be running), `AUTH_FLOW_TIMEOUT`, and a session refresh that timed out or could not reach the auth service (`TIMEOUT` / `CONNECTION_ERROR`; a slow auth service is NOT a dead login -- re-run, do not re-login) (since v0.78.0) |
