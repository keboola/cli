"""Tests for the additive ``http_auth`` plumbing across the HTTP clients (0.80.0).
...
1. **Omitting it changes nothing.** ``http_auth=None`` must be byte-identical
   to the pre-0.80.0 behaviour -- the static token still goes out as
...
class TestStaticModeUnchanged:
    def test_static_token_header_still_sent(self, httpx_mock) -> None:
        """Without http_auth, the client behaves exactly as before 0.80.0."""
