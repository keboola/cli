...
CHANGELOG: dict[str, list[str]] = {
    "0.79.0": [
        ...
        "self-contained PyInstaller binary delivered by Chocolatey, WinGet, Homebrew, "
        "apt, dnf or a signed zip -- but nothing in the code detected the difference. "
        "The release workflow freezes with `pyinstaller --collect-all keboola_agent_cli`, "
        "and `collect_all()` is a superset of `--copy-metadata`, so the whole "
        "`.dist-info` is bundled and `importlib.metadata` resolves a real version inside "
        "the binary -- the `0.0.0-dev` fallback never fires. Both update paths therefore "
        'planned `uv tool install --force --reinstall "keboola-cli[server] @ ..."`, '
        "which cannot upgrade a package-manager-owned binary: it installs a SECOND, "
        "unrelated kbagent into the uv tool directory, which usually precedes the "
        "package manager's directory on PATH -- so the user silently starts running a "
        "different install than the one `choco` / `brew` / `apt` tracks, while the real "
        "binary stays stale. On a machine with no Python at all it simply fails and the "
        "startup banner cries wolf on every invocation. The startup hook was only ever "
        "suppressed for frozen builds by accident: CI freezes from an editable install, "
        'so the bundled `direct_url.json` carries `"editable": true` and the '
        "dev-install check happened to return True. Freezing from a non-editable install "
        "(a wheel, `uv pip install .`, `UV_NO_EDITABLE=1`, or a future uv default change) "
        "flips that flag and every shipped binary starts reinstalling on startup; both "
        "states were reproduced against real PyInstaller binaries. `sys.frozen` / "
        "`sys._MEIPASS` is now detected explicitly (new `frozen_dist` module) and the "
        "kbagent self-update stage is replaced by a notification naming the channel that "
        "actually owns the binary -- identified from the binary's own path -- so the user "
        "gets `choco upgrade keboola-cli2`, `winget upgrade Keboola.KeboolaCLI2`, "
        ...
        "archive. This covers the deferred Windows helper added in 0.78.0 too: the guard "
        "sits ahead of the `should_defer()` branch, so a frozen binary is never scheduled "
        "for an install it cannot receive. The startup banner is throttled to once per "
        "version-cache TTL (it cannot resolve itself by re-exec'ing, so it would otherwise "
        "print on every command). `kbagent update` reports the same instead of running uv, "
        "and no longer misreports that deliberate refusal as a failed update; "
        "`kbagent version` advertises the channel command in place of `(run: kbagent "
        "update)`, plus an additive `install_channel` key in `--json` (absent for Python "
        "installs, so their JSON shape is byte-identical). The keboola-mcp-server stage "
        "still runs on frozen builds by design: MCP is a separate Python distribution "
