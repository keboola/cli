...
CHANGELOG: dict[str, list[str]] = {
    "0.29.0": [
        ...
        "New: `kbagent config row-create --project P --component-id C --config-id K --name NAME [--description D] [--configuration JSON|@file|-] [--is-disabled] [--branch ID]` and `kbagent config row-update --project P --component-id C --config-id K --row-id R [--name N] [--description D] [--configuration JSON|@file|-] [--set PATH=VALUE ...] [--merge] [--is-disabled | --is-enabled] [--dry-run] [--branch ID]` -- first-class lifecycle for configuration rows. `row-create` returns the full row dict with the API-assigned `id`; capture it for subsequent updates. `row-update` preserves all unspecified fields and accepts the same `--set` / `--merge` / `--dry-run` semantics as `config update`. `--is-disabled` / `--is-enabled` are mutually exclusive on `row-update` and toggle the row's active state (Storage API `isDisabled` field). Branch-aware. Closes the `create_config_row` / `update_config_row` MCP-parity gap.",
        "New: `kbagent config oauth-url --project P --component-id C --config-id K [--redirect-url URL]` -- generate the OAuth authorization URL for a component that uses OAuth. Mints a short-lived component-scoped Storage API token (1h expiry) via `create_short_lived_token()` and constructs the `https://external.keboola.com/oauth/index.html` URL with `token` + `sapiUrl` query params and `/component_id/config_id` URL fragment. Optional `--redirect-url` adds a `returnUrl` query param so the OAuth wizard redirects the user back to a custom URL after the flow completes. The returned dict includes `url`, `component_id`, `config_id`, `project_alias` (and `redirect_url` when provided). Open the URL in a browser; the OAuth flow stores the resulting credentials on the linked configuration. The OAuth host is now exposed as `OAUTH_HOST` constant in `constants.py` (per Keboola hosting convention `external.keboola.com` is the wizard host across all stacks; the per-stack difference is reflected in the `sapiUrl` parameter, not the wizard host).",
        ...
        "Permissions: new entries in `OPERATION_REGISTRY` -- `search` (read), `config.row-create` (write), `config.row-update` (write), `config.oauth-url` (read), `project.info` (read). The `--deny-writes` and `--deny-destructive` session firewalls now correctly recognize these commands. Coverage enforced by `tests/test_permissions.py::test_all_subapp_commands_registered` so future drift will fail CI.",
        "Tests: 49 new tests across 4 new test files (`tests/test_search_cli.py`, `tests/test_search_service.py`, `tests/test_config_row_cli.py`, `tests/test_config_row_service.py`, `tests/test_project_info_cli.py`, `tests/test_project_info_service.py`). All three layers covered (client / service / CLI), per-project error accumulation in textual search verified against multi-project fixtures, and `should_hint` short-circuit verified for every new command. E2E coverage: live-validated against `kbagent-e2e` and `padak` projects -- `row-create` returned `01kqz1rrmxt85e7m3ynqeq80rs`, `row-update` flipped the name and incremented `version` to 2.",
        "Plugin: `keboola-expert.md` tool selection matrix gains 5 rows for the new commands; VERSION GATE Rule 6 lists them as `0.29.0+`. New gotcha entries: `search` is a top-level command (not `config search`); options must follow the QUERY argument; `--is-disabled`/`--is-enabled` are mutually exclusive. `commands-reference.md`, `gotchas.md`, `context.py` AGENT_CONTEXT, and `CLAUDE.md` `## All CLI Commands` all updated. `SKILL.md` decision table auto-regenerates via the pre-commit hook (CI-checked).",
    ],
    "0.28.0": [
