...
CHANGELOG: dict[str, list[str]] = {
    ...
    "0.91.0": [
        ...
        "(same backend, snowflake/bigquery, full load, no external-schema bucket, alias only "
        "when column auto-sync is on, and on BigQuery not an Analytics-Hub-linked bucket); an "
        "ineligible table falls back to COPY individually and reports a per-table "
        "`clone_ineligible_reason`. `--load-type clone|copy|view` forces the type for every "
        "table and fails loudly on an ineligible choice rather than silently degrading. A "
        "table resolved or forced to COPY above 1 GiB now needs an interactive confirmation "
        "or `--force`, so a 282 GB copy can no longer start unnoticed. `--timeout SECONDS` "
        "(default 300 here) replaces the hard-coded 60 s wait, the timeout message names the "
        "job id and warns that the job keeps running server-side, and "
        "`STORAGE_JOB_TIMEOUT` now exits 4 (retryable) instead of 1.",
        "New (#695): the `notification` group is no longer read-only -- `create`, `delete` "
        "and `replace-recipient` can now fix what the #600 audit finds. `--address` carries "
        "the email address or the webhook URL, discriminated by `--channel`. "
        "`replace-recipient` is delete+recreate (the API has no update primitive): the new "
        "subscription is created first, so a new `subscription_id` is always minted and a "
        "failed delete leaves a recoverable duplicate flagged as `old_deleted: false` rather "
        "than a lost subscription. `create` without `--branch` writes no branch filter, so "
        "the subscription fires for jobs on every branch. Permission classes: `create` and "
        "`replace-recipient` are writes, `delete` is destructive.",
        "Fix (#686, #694, #696): `sync push` now stamps the manifest baseline from the API "
        "response, ending permanent phantom `~ REMOTE MODIFIED` drift. Pushed "
        "multi-statement SQL transformations were the reported symptom: push hashed the "
        "files on disk while "
        "`pull`/`diff` hash the API response, so the two sides disagreed forever. Triage "
        "found a worse sibling: with no trailing semicolons in the API's `script[]`, push "
        "silently collapsed N statements into one -- the `MULTI_STATEMENT_COUNT=1` crash "
        "shape -- while `diff` reported in sync. There is now one canonical `script[]` shape "
        "(one element = one statement), `transform.sql` carries "
        "`/* ===== STATEMENT ===== */` markers when semicolons cannot recover the "
        "boundaries, and the runtime-safety script guard also runs on the push path. An "
        "unreadable-after-write leaves the baseline untouched plus a `warnings[]` entry, "
        "never a disk-derived fallback. Entries carry `metadata.config_hash_version`; one "
        "`sync pull` per project migrates a pre-0.91.0 tree, and a tree whose only "
        "difference is the lost statement boundaries is refused per-change with "
        "`SYNC_LEGACY_BOUNDARY` telling you to pull first.",
        "Fix (#689, #691): `sync` now ignores `keboola.mcp-server-tool` and honors the "
        "manifest's `ignoredComponents` field. That field was declared in the schema and "
        "read by nothing. The MCP server's auto-created empty `mcp-workspace-<hex>` configs join "
        "`keboola.sandboxes` on the always-ignored list, and the manifest field unions with "
        "it across pull, diff and push. `pull` drops manifest entries and local directories "
        "for a newly-ignored component under the new action `ignored` (distinct from "
        "`removed` = deleted on the remote), and `diff` filters the local side too -- so a "
        "stale directory for an ignored component can no longer classify as a delete, "
        "closing the trap that destroyed production `keboola.mcp-server-tool` configs.",
        "Fix (#688, #693): `permissions set` validates every `--allow`/`--deny` pattern "
        "before persisting it. A typo or a fabricated category used to be written to "
        "`config.json` and echoed back as a normal rule while matching nothing at runtime, "
        "so the operator walked away believing the firewall had changed. Each pattern must "
        "now be a `cli:*` category, an exact operation name (including a flag-escalated "
        "string like `auth.logout --remove-projects`), or a glob matching at least one known "
        "operation; anything else is a `VALIDATION_ERROR`, exit 2, listing every offending "
        "pattern, and nothing is persisted. `permissions show` and `kbagent doctor` now flag "
        "any persisted pattern matching zero operations, not just the retired `tool:*` "
        "namespace. `PermissionEngine` itself stays lenient at evaluation time.",
        "Fix (#679, #681): `kbagent --config-dir X serve` now actually serves X. `serve` is "
        "the only subcommand with a `--config-dir` of its own and it read only that one, so "
        "the root flag was silently ignored: the wrong projects were exposed, and since "
        "0.90.1's `/auth/*` enforcement the wrong directory's permission policy applied. "
        "Precedence is now most-specific-wins -- `serve --config-dir` beats the root flag, "
        ...
        "with `/plugin marketplace add keboola/ai-kit` then "
        "`/plugin install kbagent@keboola-claude-kit`. The plugin source stays in this repo "
        "(four CI gates validate it against the live command tree), and a new "
        "`ai-kit-marketplace` release job repoints ai-kit's entry on every stable tag. This "
        "repo's own marketplace becomes a deprecated shim so existing installs keep "
        "resolving updates. `kbagent doctor` also now reports the newest plugin copy across "
        "both marketplace cache directories.",
        "Note (#697, #698): documentation and plugin guidance were brought in line with the "
        "above. Version tags moved out of markdown headings (a resolved tag changes the "
        "anchor slug and breaks inbound links), and the workspace-load skill guidance gained "
        "cheapest-first ordering, a verify-what-happened pointer at the per-table "
        "`tables[].load_type` report, and a correction of a misleading `--force` example.",
    ],
    "0.90.1": [
        "New (#675): the `serve --ui` web UI gained cross-project All Jobs and All Tokens "
        "views. All Jobs (`#/jobs-all`) is one chronological feed over every registered "
        "project with a project column, client-side credit estimates plus a header total "
        "over the shown rows, the extended status filter, a cross-project detail drawer "
        "(SSE logs, re-run, terminate) and shareable `?sel=<alias>/<jobId>` links; it polls "
        "at 15 s rather than 8 s because each tick fans out one Queue API call per project. "
        "All Tokens (`#/tokens-all`) is the read-only audit across projects with the opt-in "
        "last-used derivation sorted dormant-first globally -- clicking a row jumps to that "
        "project's Tokens page, where mint / rotate / revoke live. Both are reachable from "
        "an *All projects* button on their per-project sibling and from the command palette. "
        "Credit rates live in `web/frontend/src/config/credits.ts` and are an estimate "
        "(duration x per-component rate x container size), labelled as such in the UI.",
        "Change (#675): a multi-project `job list` now merges chronologically instead of "
        "grouping by project. Both the CLI and `GET /jobs` sort globally by the requested "
        "`--sort-by` / `--sort-order` (default `startTime desc`), missing values last in "
        "both directions, with a deterministic `(project_alias, id)` tiebreak. The old "
        "alias-then-id grouping destroyed the chronological order that a fan-out over "
        "projects exists to produce. Anything that relied on rows arriving grouped by "
        "project must group them itself.",
        "New (#675): `GET /token/list` fans out the token audit over registered projects. "
        "`project` is repeatable and, when omitted, means every project; rows are stamped "
        "with `project_alias`; a per-project failure degrades into `errors[]` instead of "
        "aborting the run; `with_last_used` is supported at its documented one-extra-call-"
        "per-token cost. Secrets are never present in the response.",
        "New (#674): the Search page was rebuilt as a search console and no longer hides "
        "skipped projects. The native multi-select and bare dropdowns became a prominent "
        "autofocused input, a names / config-bodies segmented toggle and per-type filter "
        "pills. The per-project `errors[]` the fan-out already returned -- expired session, "
        "missing `global-search` feature -- was previously dropped, so a fleet where every "
        "project was skipped rendered as a blank `0 hits` and looked broken; it now renders "
        'as a grouped "N projects skipped" notice, and a zero-project run says so instead '
        "of claiming no matches. Result rows deep-link into their home page via the same "
        "`?sel=` grammar the command palette uses.",
        "New (#672): the web UI Changelog page renders as a typed release timeline instead "
        "of a wall of raw bullet strings. Each note collapses to its first sentence and "
        "expands per note or all at once -- the web mirror of `kbagent changelog` and "
        "`--full` -- with `New` / `Fix` / `BREAKING` badges in the CLI's own colour "
        "semantics, `(#PR)` decorations turned into GitHub links, inline code chips, and a "
        "release rail that marks the running version. The `GET /changelog` payload is "
        "unchanged; the parser is a port of `headline()` and `_PREFIX_RE` and carries a "
        "keep-in-sync note.",
        "New (#677): `kbagent serve` exposes the non-interactive half of the `auth` group "
        "over REST. `GET /auth/projects` lists registrable candidates with collision-free "
        "suggested aliases, `POST /auth/register-projects` mirrors "
        "`auth register-projects --all` / `--project-id` / `--alias`, and "
        "`GET /auth/status` reports session health. `login`, `login-password` and `logout` "
        "deliberately get no endpoint (a browser is required, a password must not travel "
        "over REST, and a logout revokes machine-wide); the interactive picker stays "
        "CLI-only. No response carries a token value, not even the `kbc-session://` "
        "sentinel. `GET /auth/status` answers HTTP 200 even for a missing or expired "
        "session, reporting the health in the body's `status` field -- it is the probe you "
        "call to find out, so it must not itself fail; branch on the HTTP status only via "
        "`/auth/projects` or `/auth/register-projects`, which do 401. Closes #537.",
        "New (#677): `kbagent serve` now enforces the persisted permission policy -- on "
        "`/auth/*` routes only, so far. `create_app` composes a `PermissionEngine` from the "
        "policy of the config dir it actually serves (serve's own `--config-dir` wins) plus "
        "the `--deny-writes` / `--deny-destructive` flags, and the dependency fails closed. "
        "Do not read this as a firewall over the whole REST surface: every other router is "
        "still unguarded. Note that `kbagent --deny-writes serve` never starts at all -- "
        "`serve` is admin-class and `cli:write` spans admin -- so the reachable form is a "
        "narrow persisted policy such as "
        "`permissions set --mode allow --deny auth.register-projects`.",
        "Note (#671): `docs/web-server-endpoints.md` is now generated from the running app "
        "and gated in CI. The hand-written table in `docs/web-server.md` claimed `150+ "
        "endpoints` for a server that actually serves 227 operations across 29 routers, "
        "with ten routers -- `billing`, `token`, `notifications`, `feature`, `stream`, "
        "`dev-portal`, `ai-chat`, `semantic-layer`, `transformations`, `documentation` -- "
        "missing entirely. `make endpoints-gen` renders the reference from "
        "`create_app().openapi()`, grouped by the tag descriptions already in "
        "`server/app.py`, and `make endpoints-check` fails the build when it drifts. "
        "`web-server.md` keeps only what a generator cannot produce (architecture, auth, "
        "which surfaces are deliberately CLI-only) and no longer carries a route count. "
        "Closes #656.",
        "Internal (#670, #678): the release checklist's `vNEXT` placeholder step is now "
        "enforced by CI instead of a hand-run grep. A `vNEXT` inside an inline-code span is "
        "prose quoting the token; outside one it is a live gate -- validated at 16 real "
        "gates and 4 prose mentions against the pre-0.90.0 tree, with no allowlist. The "
        "gate arms only on a PR that raises `pyproject.toml`'s version, compared with PEP "
        "440 ordering so a stale feature branch behind a released `main` is never told to "
        "delete a placeholder the process requires it to write. `make vnext-check` is the "
        "local form and is deliberately not in `make check`. #678 then removed the pytest "
        "mirror of the same assertion, which had no base version to compare against and so "
        "turned `main` and every open PR red the moment the next cycle's first placeholder "
        "landed.",
        "Internal (#676): CI runs the test suite in parallel and stops paying for "
        "superseded runs. `pytest-xdist -n auto` on all three test jobs (the ~6k tests are "
        "in-process `CliRunner` calls with mocked HTTP and no shared mutable state), a "
        "concurrency group that cancels superseded PR runs but never a `main` push, "
        "coverage and the Python 3.13 leg moved to `main` pushes only, three redundant "
        "Windows steps dropped, and a uv cache keyed on `uv.lock`. Measured ~39 to ~15 "
        "billed minutes per run and ~9.5 to ~4 minutes wall clock; `make test` now also "
        "runs `-n auto` locally, taking `make check` from ~3.5 min to ~53 s. Path filtering "
        "for docs-only PRs was measured and rejected -- the suite reads `plugins/**`, "
        "`CLAUDE.md`, `README.md` and `scripts/**`, leaving a safe skip set that zero of "
        "the last 58 commits on `main` qualified for.",
        "Internal (#673): the release pipeline's artifact and GitHub-release actions moved "
        "off the deprecated Node 20 runtime. `actions/upload-artifact` v4 to v7, "
        "`actions/download-artifact` v4 to v8 and `softprops/action-gh-release` v2 to v3 -- "
        "each major chosen by reading `runs.using` from the pinned tag's own `action.yml`, "
        "because the naive v4 to v5 bump on either artifact action still declares node20 "
        "and would have kept emitting the same annotation. Every `uses:` in `.github/` was "
        "audited; the rest already run on node24, docker or composite.",
    ],
    "0.90.0": [
    ...
    "0.89.0": [
        ...
        "Fix (#649): `sync diff` / `sync push` are now scoped to a single branch tree. "
        "After a `sync pull --branch <dev>` re-targets every `manifest.configurations` entry "
        "at the dev branch, a production `sync diff` classified the whole orphaned `main/` "
        "tree as `added` with an empty config_id -- one `sync push` away from duplicating the "
        "entire production project (live-reproduced as `added: 19` on a 14-config project). "
        "The new `sync/branch_scope.py` adds the branch dimension the diff engine was missing: "
        "the local side is read from exactly ONE tree, and the adopt-by-id claim check "
        "(#482/#497 semantics preserved) is branch-aware -- a same-tree claim keeps the "
        "fork-by-copy CREATE, a cross-tree claim adopts when the id resolves on the target "
        "remote and otherwise lands in a new `orphaned` bucket (reasons `stale_branch_tree` / "
        "`tracked_on_other_branch`, each with a reconcile hint). Push inherits the "
        "classification, so a cross-tree entry can never be planned as a create or a remote "
        "delete; run `sync pull` to re-target the manifest. `--json` gains additive "
        "`orphaned[]` + `summary.orphaned` keys, human mode warns with a capped preview.",
        "Fix (#644): `config new --push --output-dir` no longer scaffolds a duplicate-in-waiting. "
        "The written directory used to be the PRE-POST scaffold -- no `_keboola.config_id`, "
        "always under the default branch tree even when the configuration was created in a dev "
        "branch -- so the next `sync push` read it as a brand-new configuration and POSTed a "
        "copy (the 34-duplicate incident in issue #644). The scaffold now records the created "
        "config's ID (quoted, so a numeric ID stays a YAML string) and lands in the subtree of "
        "the branch the config was ACTUALLY created in, registering an unseen branch in the "
        "manifest exactly like `sync pull --branch` does (new `sync/branch_registry.py`). With "
        "an explicit `--configuration` body the local files mirror the pushed encrypted body the "
        "way `sync pull` would materialize it. Placement degrades to `branch-{id}/` with a "
        "warning rather than falling back to the default tree. `--json` gains additive "
        "`local_scaffold` + `warnings[]` keys; scaffold-only mode (no `--push`) is byte-for-byte "
        "unchanged.",
        "Fix (#650): a scaffolded `keboola.flow` config can finally be pushed from disk. The flow "
        "scaffold was the only category without the `_keboola: {component_id}` footer, so "
        "`sync push` resolved it as component `unknown`; worse, it kept `phases` / `tasks` at the "
        "top level of `_config.yml`, which `local_config_to_api` drops -- a flow that did get "
        "pushed would have carried an EMPTY configuration. The scaffold now emits `version: 2`, "
        "nests the flow definition under `_configuration_extra` (the shape a pulled flow "
        "round-trips through) and carries the `_keboola` footer.",
        ...
        "issue #585.",
        "Internal (#648): version bumps and changelog entries now happen ONLY in a dedicated "
        "release PR. Feature PRs used to bump `pyproject.toml` themselves, so parallel PRs "
        "collided on every merge and the merge order silently renumbered releases -- the "
        "18-entry `KNOWN_UNRELEASED` list in `scripts/generate_changelog.py` is the accumulated "
        "damage. A feature PR now tags version-gated documentation with the literal placeholder "
        "`vNEXT` (`make version-gate-check` ignores it but still rejects a guessed numeric "
        "version), and the release PR replaces every placeholder with the version it ships. "
        "Documented in `CLAUDE.md` > Versioning and `CONTRIBUTING.md` > Releasing a new version; "
        "a beta stays the one exception where the bump rides the feature branch.",
    ...
    "0.88.0": [
        ...
        "`describe-batch` stored each description as a flat "
        "`KBC.column.{name}.description` entry on the TABLE's metadata -- a convention "
        "nothing but kbagent itself ever read, so a documented column looked blank in "
        "the UI, was invisible to `get_tables` in the MCP server, and never reached the "
        "Snowflake `COMMENT` / BigQuery column description. The write now goes through "
        "the native `PUT /v2/storage/branch/{branch}/tables/{id}/definition` endpoint "
        "(the same one the web UI uses; async, waits for the `tableDefinitionUpdate` "
        "storage job), and the backend mirrors the value into `columnMetadata` "
        "`KBC.description` for typed AND untyped tables -- one write, visible "
        "everywhere. `description: null` clears a column description.",
        "New (#624): every native write sets `isDescriptionSystemManaged: false`. "
        "That flag is what stops the next component run's Output Mapping from "
        "overwriting a hand-authored description (DMD-1662). The old metadata-key "
        "write had no such protection.",
        "New (#624): `kbagent storage describe-migrate` converts legacy flat "
        "`KBC.column.*` entries to the native endpoint in bulk. Full usage: "
        "`describe-migrate --project ALIAS [--table-id ID ...] [--bucket-id ID] "
        "[--prune-orphans] [--dry-run] [--yes] [--branch ID]`. Scope is "
        "explicit tables, one bucket, or the whole project; it scans first and asks for "
        "confirmation before writing (`--dry-run` reports without touching anything), and "
        "per-table failures are accumulated instead of aborting the run. A column that "
        "already carries a different visible description is skipped as a `conflict` "
        "(newer value wins), and an entry for a column that no longer exists is skipped "
        "as an `orphan` unless `--prune-orphans` is passed. `describe-column` / "
        "`describe-batch` also migrate any remaining legacy entries on the table they "
        "touch, as part of the same write. Migrated flat entries are DELETED afterwards, "
        "so clearing a description later cannot be silently resurrected by the read "
        "fallback.",
        "New (#624): `storage table-detail` now reads column descriptions written by "
        "the UI or by a component, not only kbagent's own. Resolution precedence is "
        "native definition -> `columnMetadata` `KBC.description` -> legacy "
        "flat `KBC.column.*` (alias tables fall back to the source table's "
        "`columnMetadata`, matching the MCP server). The response always carries a "
        "`legacy_column_descriptions` list naming the columns still backed by the legacy "
        "convention; human output prints a warning pointing at `describe-migrate`. "
        "Reading never writes -- a read-only token or a `--deny-writes` session is "
        "unaffected.",
        "Change (#624): `describe-column` / `describe-batch` now REJECT a column name "
        "that does not exist on the table, before any write. The old flat-metadata write "
        ...
        "Internal: two release-drift gates. `make version-gate-check` (new, and in CI) rejects a `(since vX.Y.Z)` / `X.Y.Z+` marker naming a version with no changelog entry. `keboola-expert.md` turns those markers into a hard refusal rule, so one naming a version that never ships makes the agent refuse a flag the user actually has -- which is exactly what a release renumber produces, in bulk, with nothing noticing. It covers 418 markers across 71 versions today. `make changelog-check` gains the inverse of the audit it already ran: a changelog entry with no published release. Publishing is a manual step while the version bump rides along with any PR touching `pyproject.toml`, so a merge-train can strand a whole block and the next tag absorbs it unannounced. That has happened 18 times against 161 releases; those are frozen as a baseline the check stops from growing.",
    ...
    "0.86.0": [
        ...
        "in the flow's `configuration` JSON, so `flow detail` / `config detail` never "
        "showed them -- they were the last unauditable notification surface in a "
        'fleet-wide "are our alert recipients still valid" sweep (issue #600). The '
        ...
        "`project_wide_excluded`, with a warning in human mode. Event names are kebab-case "
        ...
        "Fix (#612): `sync diff` / `sync status` stopped reading GCP and Azure ciphertext "
        "as plaintext. `sync/secrets.py::ENCRYPTED_PREFIXES` listed only the four AWS-form "
        "prefixes, so on a GCP or Azure stack `is_encrypted_value()` returned False for a "
        "perfectly good `KBC::ProjectSecureGKMS::` / `KBC::ProjectSecureKV::` value. The "
        "tuple is now derived from the platform's cipher registry "
        "(keboola/keboola-operator `internal/encryptor/wrapper/registry.go`): 8 scopes x 3 "
        "clouds plus the 3 pre-2019 `KBC::*Encrypted==` ciphers, 27 prefixes in total, with "
        "a test that fails if the family drifts. Scope of the impact was narrow -- "
        "`is_encrypted_value` has a single caller, the ciphertext redaction in "
        "`sync/diff_engine.py::_normalize`, and the `#`-key check already covered every "
        "conventionally-named secret; the plaintext audit behind `sync status` / `doctor "
        "sync_secrets` never used this helper (it matches the generic `KBC::` prefix and "
        "was always cloud-agnostic). One-time effect on GCP/Azure working trees that hold "
        "an encrypted value under a NON-`#` key: that value now normalises to the "
        "`<ENCRYPTED>` placeholder, so its config hashes once as changed on the first run "
        "after the upgrade.",
    ...
    "0.84.2": [
        ...
        "New: `kbagent config state-get`/`state-set` read and write a configuration's "
        "runtime `state` (closes #593). kbagent now calls the dedicated Storage API "
        "state endpoint it never used before. `state-get --project P --component-id C --config-id ID "
        "[--row-id R] [--branch ID]` returns the root state, or a row's state with "
        "`--row-id` (a missing row id fails loudly, it does not return an empty dict). "
        "`state-set --state JSON|@file|- [--row-id R] [--branch ID] [--dry-run] [--yes]` "
        "writes it: the payload must be a JSON object under 4 MB (validated before the "
        "round-trip), `--dry-run` previews a current-vs-new diff without writing (same "
        "shape as `config update --dry-run`), and a state identical to the current one "
        "short-circuits with `changed: false` and no API call. `state-set` is a guarded "
        "write -- it prompts for confirmation unless `--yes` or `--json` is passed, "
        "matching every other `config` write. The motivation: state controls what an "
        "incremental component considers already processed, so backfills, resets after a "
        "bad run, and seeding a dev branch (which always starts with `state: {}`) were all "
        "reachable only by hand-editing in the Keboola UI. The case that forced this: "
        "Keboola is retiring `processed_tags`/`query` in file input mapping in favor of "
        "`changed_since: adaptive`, and `adaptive` with an empty state reloads the ENTIRE "
        "file history instead of just new files -- on a dev branch that always starts "
        "empty, testing the migration safely requires seeding a known `lastImportId` "
        "first, which is now scriptable end-to-end (`branch create` -> edit mapping -> "
        "`sync push` -> `state-set` seed -> `job run` -> `state-get` verify -> `branch "
        "merge`). See the config-state-workflow reference for the full playbook. Row "
        "support matters here: for row-based components the root `state` node is unused, "
        "so `--row-id` is the primary path, not a garnish. Permissions: "
        "`config.state-get` (read), `config.state-set` (write).",
        "Fix (#593): `config update --set` now exits 2 when a path targets a top-level API "
        "field instead of `configuration`. Previously such a write silently landed in "
        "`configuration.state.*` and did nothing. This covers `config row-update --set` "
        "too. The rejected first segments are (`state`, `rows`, `name`, "
        "`description`, `id`, `version`, `currentVersion`, `changeDescription`, `created`, "
        "`creatorToken`, `isDeleted`, `isDisabled`). Before this fix, `--set "
        "'state.x=y'` silently wrote to `configuration.state.x` instead of the real "
        "`state` field: the command exited 0, bumped the config version, and even showed "
        "a plausible-looking `--dry-run` diff, while the runtime state stayed completely "
        "untouched -- a write that looked successful at every observable layer while "
        "doing nothing. The guard fires before any network call, so `--dry-run` is "
        "rejected too. The error names the offending path and routes to the right tool: "
        "`state.*` -> `config state-set`, `name` -> `--name`, `description` -> "
        "`--description`, a row's `isDisabled` -> `row-update --is-disabled/--is-enabled` "
        "(the config-level flag isn't settable via `--set` at all), anything else -> "
        '"not settable via --set"; a genuine `configuration.<prefix>` key can still be '
        "written via `--configuration JSON|@file|-`. A plain `--set 'parameters.x=y'` is "
        "unaffected -- only a first segment colliding with a sibling field is rejected. "
        "Also fixed: `set_nested_value` no longer silently accepts bracket syntax like "
        "`files[0]` as a literal dict key -- it now raises a clear error pointing at the "
        "working `files.0` form (unchanged, still the way to index an existing list).",
    ],
    "0.84.1": [
