...
flat unless deliberately migrated. That is normative intent, not just a
...
class StorageRequester(Protocol):
    ...
    def wait_for_storage_job(
        ...
        """Poll a Storage job to a terminal state; RAISE if it failed.

        Not a description of the current implementation but a REQUIREMENT on
        any future one: a failed job must raise ``STORAGE_JOB_FAILED``,
        whether the failure arrives in the ``job`` passed in (the Storage API
        can fail fast, never reporting ``waiting``) or in a polled response.
        ``merge()`` does not re-check the returned job, so an implementation
        that *returns* a failed job turns a failed merge into a silent
        success. This blind spot was real house-wide until #603 (DMD-1898)
        made the poller check-then-fetch; do not reintroduce it under this
        Protocol.
        """
        ...
...
class MergeRequests:
    ...
    def merge(self, merge_request_id: int) -> dict[str, Any]:
        ...
        ``STORAGE_JOB_FAILED`` instead of masquerading as success -- for a
        fast fail (terminal already in the 202 body) as much as for one that
        shows up while polling, because the poller checks before it fetches
        (#603); this method deliberately does not re-check, see
        ``StorageRequester.wait_for_storage_job``. The MR state stays
        pollable via ``get()``.
