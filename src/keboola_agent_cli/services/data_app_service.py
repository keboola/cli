...
def create_synced_data_app(
    ...
    *,
    ...
    ``sync push`` / ``sync clone`` create configs through the Storage API
    only (``create_config``). A ``keboola.data-apps`` config has a second
    half — the Data Science ``/apps`` deployment record — and the runtime
    type (``python-js`` / ``streamlit`` / ...) lives ONLY on that record,
    never in the Storage config body. A plain ``create_config`` therefore
    leaves the platform to lazily mint the DS record under its default type,
    so a cloned ``python-js`` app deploys as ``streamlit``.
    ...
    via ``update_config``. The ``parameters.id`` back-pointer is repointed at
    the newly assigned app id — the cloned body still carries the SOURCE
    project's app id, which is meaningless in the target.

    Returns the ``update_config`` response (the Storage config, whose ``id``
    is the newly assigned config ULID) so the caller's manifest writeback is
    identical to the ``create_config`` path.
    """
    ...
    # Repoint the DS back-pointer at the target app; the cloned body still
    # carries the source project's app id.
